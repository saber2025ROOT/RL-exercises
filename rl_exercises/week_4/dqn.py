...
class DQNAgent(AbstractAgent):
    ...
    def epsilon(self) -> float:
        ...
        e = self.epsilon_final + (self.epsilon_start - self.epsilon_final) * np.exp(
    ...
    def train(self, num_frames: int, eval_interval: int = 1000) -> None:
        ...
        plt.savefig(f"dqn_h128_l3_buf{self.buffer.capacity}_batch{self.batch_size}.png")
        plt.close()
        np.savez(
            f"dqn_seed{self.seed}.npz",
            frames=np.array(frames_log),
