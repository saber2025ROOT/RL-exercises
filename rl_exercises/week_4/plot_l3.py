...
def align_curves(grouped):
    ...
    min_len = min(len(curve) for curves in grouped.values() for curve in curves)

    aligned = {}

    for name, curves in grouped.items():
        # Keep only the first min_len values
        aligned[name] = np.array([curve[:min_len] for curve in curves])
...
def plot_aggregate_metrics(grouped):
    ...
    final_scores = {name: curves[:, -1][:, None] for name, curves in grouped.items()}
