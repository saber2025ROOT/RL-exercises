...
class RNDPPOAgent(PPOAgent):
    ...
    def get_rnd_bonus(self, state: np.ndarray) -> float:
        ...
        error = F.mse_loss(predictor_emb, target_emb, reduction="none").mean(dim=1)
    ...
    def update(self, trajectory: List[Any]) -> Tuple[float, float, float, float]:
        ...
        for _ in range(self.epochs):
            for b_states, b_actions, b_oldlogp, b_adv, b_ret_ext, b_ret_int in loader:
                ...
                # Normalize observations for RND predictor training.
                obs_mean = torch.as_tensor(self.obs_rms.mean, dtype=torch.float32)
                obs_var = torch.as_tensor(self.obs_rms.var, dtype=torch.float32)
